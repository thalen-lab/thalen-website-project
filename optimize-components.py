...
# Define responsive class replacements for components
replacements = [
    ...
    # Icon sizing
    (r'h-12 w-12', 'h-8 w-8 sm:h-10 sm:w-10 md:h-12 md:w-12'),
    (r'h-10 w-10', 'h-7 w-7 sm:h-8 sm:w-8 md:h-10 md:w-10'),
    (r'h-8 w-8', 'h-6 w-6 sm:h-7 sm:w-7 md:h-8 md:w-8'),
    
    # Space-y improvements
    (r'space-y-8(?![0-9])', 'space-y-4 sm:space-y-6 md:space-y-8'),
    (r'space-y-6(?![0-9])', 'space-y-3 sm:space-y-4 md:space-y-6'),
    (r'space-y-4(?![0-9])', 'space-y-2 sm:space-y-3 md:space-y-4'),
]

# Files to process
components_dir = '/home/ubuntu/nexdyne-website/client/src/components'
...
print(f"Processed {len(processed)} component files:")
for f in processed:
